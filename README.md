...
- 页面每 60 秒刷新当前页签；五个主页签首次进入时独立并发加载，每个页签显示自己的旋转状态，已完成页签可以立即查看。
- 看板页面使用自适应版面，手机宽度下卡片、图表、页签和明细弹窗自动收缩；表格宽度超过卡片可用空间时使用横向滑动查看，避免表头和内容越过卡片边界。
- 看板默认使用浅灰页面底、白色内容面、橙色主强调和青绿色辅助色的亮色主题，并提供亮暗主题切换；用户选择保存在浏览器 `localStorage` 的 `ops-dashboard-theme` 中，图表坐标、图例和提示框会同步更新。
- 道具使用横向条形图的标题会随实际绘图区动态对齐，窗口缩放或分类标签宽度变化时保持标题与条形图起点一致。
- 每 10 分钟保存一次本地每日快照。
- 每日趋势图会用生产库近14日数据覆盖同日本地快照，快照日期按 `OPS_DASHBOARD_TIME_ZONE` 计算，避免早期或错日快照遮盖真实数据。
- 数据库暂不可用时，页面仍返回空统计并在备注中显示数据源错误，避免首屏变成 500。
- 顶部今日收入拆分为 Stripe 和 Steam 两张卡片，分别展示 CNY 金额和已完成订单数。
- 运营总览增加最近 7 日每日氪金医院清单。统计范围为看板时区内今天及前 6 天，Stripe 仅纳入 `COMPLETED` 订单，Steam 仅纳入 `COMPLETED` 且已发货订单，Paddle 作为未产生收益的兜底渠道不纳入统计；完整日期窗口始终保留，收入为零的日期显示金额、医院数和笔数均为零。明细按日期、医院、渠道和币种聚合金额，并同时展示每日汇总、7 日付费医院数、支付笔数和分币种总金额。医院名称下显示医院 ID，新增院长列在院长名下显示关联账号邮箱，缺失时显示占位文本；邮箱仅通过已有受限看板的统计接口返回。每日汇总与医院明细使用相同的固定高度；默认选中最新日期，点击或使用键盘选择左侧日期会筛选右侧明细，右侧按每组订单的最近支付时间倒序展示。
- 每日充值金额图固定展示看板时区内最近 14 个自然日，CNY 无订单的日期补为零，避免横轴跳过零收入日期。图中叠加 CNY 的 14 日移动平均线，每一点使用当日及此前 13 个自然日的金额，包含零收入日；为保证首个可见日期也有完整窗口，统计 API 读取最近 27 天订单，图表仍只显示最近 14 天。
- 运营总览精简为在线、日活、注册、支付收入、最近 7 日付费医院、日趋势和道具使用；元宝购入、元宝消耗、道具购买榜及元宝明细从首页移入独立元宝统计页。
- 日活与注册趋势仅展示看板时区内今天及前 6 天。日活优先采用进入游戏日志，历史快照只补充日志缺失的日期，避免日志保留范围外的旧日期显示为零；日活和注册分别使用左右纵轴。快照写入会提交 SQLite 事务，其他 14 日趋势保持原范围。
- 图表下方提供道具、金钱、声望、公会、注册者统计页签，每页支持 20、50、100 条分页。
- 页面刷新按钮显示当前页签状态；后台加载中的其他页签在页签按钮上独立显示旋转状态。
- 道具页签按全量日志聚合商品购买数量、消耗数量和元宝使用数量；金钱、元宝、声望、公会按当前值从高到低排序；注册者按注册时间从新到旧排序。
- 页面顶部依次提供站点级页签：`运营总览` 展示精简运营指标，`元宝统计` 展示全周期元宝增减和消费点，`特需门诊` 展示常见病特需门诊只读分析，`医托拉人` 展示医托钱包改档后的普通拉人、钱包和名片反拉数据，`跳蚤市场` 展示洗手间交易池的库存、流转和参与医院统计。
- 元宝统计从 `t_log_yuanbao` 最早记录月与版本事件最早月份中的较早月份开始生成。只有版本事件且尚无元宝总账的首发月份保留为零值月份，使月度序列覆盖最初上线到现在。新增量为正向余额变化扣除卫生间匿名交易出售转入，消耗量为负向余额变化扣除卫生间匿名交易购买和公会捐献转出；付费购入量来自 Stripe 已完成订单及 Steam 已完成且发货订单的 `yuanbao_amount`，Paddle 不纳入购入量。月度曲线同时展示新增、消耗、非消耗转移、其中付费购入和净增减。
- 元宝统计提供“包含平台主动赠送元宝”勾选项，默认包含并在浏览器保存选择。平台主动赠送按增加原因中包含“补偿”“赠送”或“礼包”识别；加速任务奖励属于玩家完成任务所得，充值和玩家交易也不归入主动赠送。取消勾选后，累计增加、净变化、月度曲线、当月主要增加事件和增加来源表同步排除这部分影响，消耗口径保持不变。
- 元宝消费点覆盖总账内造成元宝从系统中消失的扣减原因。商店购买合并同一道具的不同购买数量并归为间接消耗，其他造成元宝消失的明确扣减归为直接消耗。卫生间匿名交易属于玩家间转移，公会捐献属于玩家到公会的转移，两者从消费点和消耗总额排除，并按月单列非消耗转移。卫生间匿名交易出售也从新增来源和新增总额排除。页面列出每个消费点的累计消耗、占比、记录数、医院数和首次、最近月份。月度大事件同时展示新增消费点、非消耗转移、当月主要消耗来源、主要增加来源，以及游戏根目录 `CHANGELOG.md` 中的新玩法和新功能上线记录。版本事件按日志实际日期归月，先显示最近两项，其余项目可展开查看；管理后台、部署配置、测试和纯修复条目不归入玩家版本事件。
- 看板携带 `app/data/gameplay_release_events.json` 版本事件快照。游戏更新日志变化后，在看板仓库执行 `python scripts/sync-changelog-events.py C:\workspace\hospital-backend\CHANGELOG.md` 刷新快照，测试、提交并发布看板后生效。
...
- 跳蚤市场页增加当前降权挂单、累计扔到大街和累计被乞丐捡取三组统计。降权明细逐条展示当前活跃挂单的卖家医院、院长、物品、数量、整单售价、计价货币、道具单价或金钱兑换比例及已存在的最低状态，不展示造成降权的查看医院；生产记录没有独立归零原因时省略原因列。入街与捡取按道具和金钱内容分别汇总记录数、数量、参与医院及最近发生时间。
...
- 跳蚤市场统计只读查询 `t_toilet_market_listing`、`t_toilet_market_offer_exposure`、`t_toilet_market_transaction`、`t_toilet_street_item` 和 `t_hospitals`。
