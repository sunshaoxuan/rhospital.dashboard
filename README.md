...
- 运营总览增加最近 7 日每日氪金医院清单。统计范围为看板时区内今天及前 6 天，Stripe、Paddle 仅纳入 `COMPLETED` 订单，Steam 仅纳入 `COMPLETED` 且已发货订单；明细按日期、医院、渠道和币种聚合金额，并同时展示每日汇总、7 日付费医院数、支付笔数和分币种总金额。明细与每日汇总使用固定高度滚动区域。
