...
- 元宝统计从 `t_log_yuanbao` 最早记录月开始生成连续月份，余额增加量为 `new_value > old_value` 的差额合计，消耗量为 `old_value > new_value` 的差额合计，付费购入量来自 Stripe 已完成订单及 Steam 已完成且发货订单的 `yuanbao_amount`。Paddle 不纳入购入量。月度曲线同时展示全部增加、全部消耗、其中付费购入和净变化。
...
- 元宝消费点覆盖总账内全部扣减原因。商店购买合并同一道具的不同购买数量并归为间接消耗，其他明确扣减归为直接消耗；页面列出每个消费点的累计消耗、占比、记录数、医院数和首次、最近月份。月度大事件按新增消费点、当月主要消耗来源和主要增加来源客观生成，并保留全部元宝增加来源用于核对补偿、奖励、充值和市场出售等变化。
