...
- 元宝统计从 `t_log_yuanbao` 最早记录月开始生成连续月份。新增量为正向余额变化扣除卫生间匿名交易出售转入，消耗量为负向余额变化扣除卫生间匿名交易购买和公会捐献转出；付费购入量来自 Stripe 已完成订单及 Steam 已完成且发货订单的 `yuanbao_amount`，Paddle 不纳入购入量。月度曲线同时展示新增、消耗、非消耗转移、其中付费购入和净增减。
